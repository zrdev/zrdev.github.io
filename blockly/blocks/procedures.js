...
Blockly.Blocks['procedures_defnoreturn'] = {
	...
	updateParams_: function() {
		...
		for (var x = 0; x < this.arguments_.length; x++) {
			if (hash['arg_' + this.arguments_[x].name.toLowerCase()]) {
				badArg = true;
				break;
			}
			hash['arg_' + this.arguments_[x].name.toLowerCase()] = true;
		}
		if (badArg) {
	...
	mutationToDom: function() {
		...
		for (var x = 0; x < this.arguments_.length; x++) {
			...
			parameter.setAttribute('type', this.arguments_[x].type);
			parameter.setAttribute('name', this.arguments_[x].name);
		...
		for (var x = 0, childNode; childNode = xmlElement.childNodes[x]; x++) {
			if (childNode.nodeName.toLowerCase() == 'arg') {
				this.arguments_.push({
					...
					isArray: childNode.getAttribute('isArray'),
					length: Infinity
				});
			}
		}
	...
	decompose: function(workspace) {
		...
		for (var x = 0; x < this.arguments_.length; x++) {
			...
			paramBlock.initSvg();
			paramBlock.setFieldValue(this.arguments_[x].type, 'TYPE');
			paramBlock.setFieldValue(this.arguments_[x].name, 'NAME');
			paramBlock.setFieldValue(this.arguments_[x].isArray, 'ISARRAY');
	...
	compose: function(containerBlock) {
		...
		while (paramBlock) {
			this.arguments_.push({
				...
				isArray: paramBlock.getFieldValue('ISARRAY')
			});
	...
	renameVar: function(oldName, newName) {
		var change = false;
		for (var x = 0; x < this.arguments_.length; x++) {
			if (Blockly.Names.equals(oldName, this.arguments_[x].name)) {
				this.arguments_[x].name = newName;
				change = true;
			}
		}
		if (change) {
	...
	customContextMenu: function(options) {
		...
		for (var x = 0; x < this.arguments_.length; x++) {
			...
			xmlArg.setAttribute('type', this.arguments_[x].type);
			xmlArg.setAttribute('name', this.arguments_[x].name);
			xmlArg.setAttribute('isArray', this.arguments_[x].isArray);
		...
		for (var x = 0; x < this.arguments_.length; x++) {
			var option = {enabled: true};
			var name = this.arguments_[x].name;
...
Blockly.Blocks['procedures_defreturn'] = {
	...
	getProcedureDef: function() {
		...
		return [this.getFieldValue('NAME'), this.getArgList(), true];
	},
...
Blockly.Blocks['procedures_definit'] = {
	// Define the special init function.
	init: function() {
		this.setHelpUrl(Blockly.Msg.PROCEDURES_DEFNORETURN_HELPURL);
		this.setColour(290);
		this.appendDummyInput()
		...
		this.appendStatementInput('STACK')
				.setCheck('statement');
		this.setTooltip('This code runs once at the start of the match. Use it to declare and initialize global variables.');
		this.arguments_ = [];
		//The function block on a page cannot be deleted
		this.setDeletable(false);
		this.setMovable(false);
...
Blockly.Blocks['procedures_defloop'] = {
	// Define the special loop function.
	init: function() {
		this.setHelpUrl(Blockly.Msg.PROCEDURES_DEFNORETURN_HELPURL);
		this.setColour(290);
		this.appendDummyInput()
		...
		this.appendStatementInput('STACK')
				.setCheck('statement');
		this.setTooltip('Main loop. This code runs once per second.');
		this.arguments_ = [];
		//The function block on a page cannot be deleted
		this.setDeletable(false);
		this.setMovable(false);
...
Blockly.Blocks['procedures_mutatorarg'] = {
	...
	init: function() {
		...
		this.appendDummyInput()
				...
				.appendField(' ')
				.appendField(new Blockly.FieldCheckbox('FALSE'), 'ISARRAY')
	...
	onchange: function() {
		...
		do {
			if (block.type === 'procedures_defnoreturn' ||
					block.type === 'procedures_definit' ||
					block.type === 'procedures_defloop' ||
