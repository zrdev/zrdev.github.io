...
class Gen_compressed(threading.Thread):
  ...
  def gen_core(self):
    ...
    filenames = calcdeps.CalculateDependencies(self.search_paths,
        [os.path.join('blockly', 'core', 'blockly.js')])
    for filename in filenames:
      # Filter out the Closure files (the compiler will add them).
      if filename.startswith('closure-library-read-only'):  # '../'
        continue
      f = open(filename)
  ...
  def gen_blocks(self):
    ...
    filenames = glob.glob(os.path.join('blockly', 'blocks', '*.js'))
    for filename in filenames:
      f = open(filename)
      params.append(('js_code', ''.join(f.readlines())))
      f.close()
  ...
  def gen_ang(self):
    target_filename = 'zr_compressed.js'
    # Define the parameters for the POST request.
    params = [
        ('compilation_level', 'SIMPLE_OPTIMIZATIONS'),
        ('output_format', 'json'),
    ...
    # Read in all the source files.
    filenames = glob.glob(os.path.join('js', '*.js')) + glob.glob(os.path.join('js', '*', '*.js'))
    for filename in filenames:
      f = open(filename)
      params.append(('js_code', ''.join(f.readlines())))
      f.close()
  ...
  def gen_generator(self, language):
    ...
    filenames = glob.glob(
        os.path.join('blockly', 'generators', language, '*.js'))
    filenames.insert(0, os.path.join('blockly', 'generators', language + '.js'))
    for filename in filenames:
      f = open(filename)
      params.append(('js_code', ''.join(f.readlines())))
      f.close()
...
http://code.google.com/p/blockly/wiki/Closure""")
  ...
  search_paths = calcdeps.ExpandDirectories(
      ['closure-library-read-only', os.path.join('blockly', 'core')])
