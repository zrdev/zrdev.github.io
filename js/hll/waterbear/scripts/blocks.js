...
$(document).ready(function(){
	var tmp = document.getElementById('color_contents');
    window.cw = Raphael.colorwheel(tmp, 300, 180);
});
...
function Block(options){
    ...
    var opts = {
        ...
        type: null,
		tabId: null // specify which tab this belongs under
    };
    $.extend(opts, options);
    if (opts.trigger){
    ...
    var wrapper = $('<span class="wrapper ' + opts.klass + '"><span class="block"><span class="blockhead"><label>' + Label(labels[0]) + '</label></span></span></span>');
	...
	if (i > 0 && labels.length > i) {
		block.append('<span class="blockhead"><label>'+Label(labels[i])+'</label></span>');
	}
...
function choice_func_orig(s, listname, default_opt){
...
function choice_func_noopt(s, listname){
    return choice_func(s, listname, '');
}
...
function choice_func(s, listname, default_opt){
    ...
    if (listname == 'arrays' &&
	default_opt != undefined &&
	default_opt != '' &&
	/^([0-9]+)$/.test(default_opt)) {
	datastr = ' data-card="'+default_opt+'"';
	mincard = default_opt;
    }

    return '<span class="string ' + listname + ' autosocket"' + datastr + '><select>' + 
        list.map(function(item){
	    valuetypecard = item.split(':');
	    value = valuetypecard[0];
	    label = value.replace(/\[.*\]/g,'');
	    type = 'dynamicvar type-'+valuetypecard[1];
	    card = valuetypecard[2];
	    if (mincard != '' && parseInt(card) < parseInt(mincard)) {
		return '';
	    }
            if (value === default_opt){
                return '<option selected label="'+label+'" value="'+value+'" class="'+type+'" data-card="'+card+'">'+label+'</option>';
            }else{
                return '<option label="'+label+'" value="'+value+'" class="'+type+'" data-card="'+card+'">'+label+'</option>';
            }
        }).join('') +
        '</select></span>';
}
...
function Label(value){
    ...
    // FIXME: Move specific type handling to raphael_demo.js
    value = value.replace(/\[constant:(-?\d*\.?\d+)\]/g, '<span class="number constant socket"><input type="number" value="$1"></span>');
    value = value.replace(/\[constant\]/g, '<span class="number constant socket"><input type="number"></span>');
    ...
    value = value.replace(/\[boolean:(true|false)\]/g, '<span class="boolean socket"><select><option class="type-bool">true</option><option class="type-bool" selected>false</option></select></span>');
    value = value.replace(/\[boolean\]/g, '<span class="boolean socket"><select><option class="type-bool">true</option><option class="type-bool">false</option></select></span>');
    ...
    value = value.replace(/(?:\[choice\:)(\w+)(?:\])/g, choice_func_noopt);
    return value;
}
